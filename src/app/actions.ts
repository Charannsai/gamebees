...
export async function createBooking(formData: {
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      sendBookingNotificationEmail(data[0], itemName)
        .then((res) => console.log("[DEBUG] sendBookingNotificationEmail result:", res))
        .catch((err) => {
          console.error("[DEBUG] Error sending booking email:", err);
        });
    } else {
